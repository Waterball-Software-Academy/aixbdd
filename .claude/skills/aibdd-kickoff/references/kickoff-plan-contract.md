...
|---|---|---|
| `q1-tech-stack` | Pick project stack | exactly two selectable options (`python_e2e` / `java_e2e`), no `Other` |
...
| `q4-codebase-layout` | Decide whether the boundary codebase lives at `${PROJECT_ROOT}` or `${PROJECT_ROOT}/${BOUNDARY_CODEBASE_SUBDIR}` | two options (`repo_root` / `subdir`); `subdir` requires a kebab-case directory name |
