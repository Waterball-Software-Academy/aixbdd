...
Kickoff 的 Q1 目前提供 Python、Java、TypeScript / Next.js 三個可選答案；其他 frontend 框架（Vue / Svelte 等）、Unit Test only、Mobile 只可在說明文字中提到「尚未支援」，不得放進 AskUserQuestion options。Q1 不得提供 `Other` / 自由輸入選項。
...
Q1 選 TypeScript / Next.js 後固定為 Playwright BDD + Storybook component contracts（playwright-bdd + Playwright + Storybook addon-vitest / addon-a11y）。
...
|---|---|---|---|
| `TLB_ID` | backend stack 預設 `backend`；Next.js stack 預設 `frontend` | `course-api` / `course-ui` | kebab-case，寫入 `boundary.yml` 的 boundary id；Java stack 同時直接成為 Maven `<artifactId>`；Next.js stack 同時直接成為 `PROJECT_SLUG` |
| `BOUNDARY_TYPE` | backend stack: `web-service`；Next.js stack: `web-app` | `web-service` / `web-app` | 由 Q1 stack 決定 |
| `BOUNDARY_ROLE` | backend stack: `backend`；Next.js stack: `frontend` | `backend` / `frontend` | 由 Q1 stack 決定 |
...
| `FRONTEND_SUBDIR` | `""` | `frontend` | 空字串 = frontend 與 specs 直接放在 repo root；非空 = 都掛在 `${PROJECT_ROOT}/${FRONTEND_SUBDIR}/` 下 |

Q4（佈局決定）必須在 Q3（service/app name）之後、推導確認之前發問。`subdir` 模式須收集自訂目錄名（kebab-case 建議）；backend stack 更新 `arguments.yml` 的 `BACKEND_SUBDIR`，Next.js stack 更新 `FRONTEND_SUBDIR`。下游 `/aibdd-auto-starter` 的 same-repo guard 會以 `${PROJECT_ROOT}/<selected-subdir>/${AIBDD_ARGUMENTS_PATH}` 為合法位置。
...
TypeScript / Next.js 範例：

```text
Q1: nextjs_playwright
Q2: zh-hant
Q3: frontend
Q4: repo_root
```
...
- TypeScript + Next.js 16 + React 19 + Storybook 10 + Tailwind 4 + Zod 4 + playwright-bdd + Playwright

| 選項 | 說明 |
|---|---|
| A | python_e2e — Python 後端 stack |
| B | java_e2e — Java 後端 stack |
| C | nextjs_playwright — TypeScript / Next.js 前端 stack |

其他 frontend 框架、Unit Test only、Mobile 尚未支援；本輪不提供這些選擇。
```
...
### Q3 唯一 top-level boundary 名稱

```markdown
[Q3/4] 這個 top-level boundary 要叫什麼名字？

建議：
- Python：<project>-api
- Java：專案小寫名（會直接作為 Maven artifactId）
- Next.js：<project>-ui 或 `frontend`（會直接作為 PROJECT_SLUG）

請提供 kebab-case 名稱（如 course-api 或 course-ui），或直接回答 `backend` / `frontend` 接受預設。
```
...
預設是 repo root（選定 stack 與 specs/ 直接掛在 repo 最外層）。
若選 subdir：
...
- Next.js：所有 frontend code（src/、features/、.storybook/、playwright.config.ts）與 specs/ 會掛在 ${PROJECT_ROOT}/${FRONTEND_SUBDIR}/ 下。

| 選項 | 說明 |
|---|---|
| A | repo_root：選定 stack 在 repo root，對應 subdir="" |
| B | subdir：選定 stack 在子目錄，請另提供子目錄名（kebab-case，例如 `backend` 或 `frontend`） |
...
| 區塊 | 必須呈現的白話內容 | 禁止事項 |
|---|---|---|
| Stack 佈局 | 選定 stack 的 filesystem 根目錄絕對路徑（backend: `${PROJECT_ROOT}` 或 `${PROJECT_ROOT}/${BACKEND_SUBDIR}`；frontend: `${PROJECT_ROOT}` 或 `${PROJECT_ROOT}/${FRONTEND_SUBDIR}`），所有後續路徑都掛在此根之下 | 不可只列 `BACKEND_SUBDIR` / `FRONTEND_SUBDIR` 變數名 |
| 專案骨架 | AIBDD 規格文件會放在哪裡、架構文件會放在哪裡 | 不可只列 `SPECS_ROOT_DIR`、`ARCHITECTURE_DIR` |
| Boundary 文件 | 唯一 boundary 會有 contracts / data / shared / packages（root） / test-strategy skeleton；packages 底下的功能模組目錄交由 Discovery | 不可預設展示全部 boundary-aware / late-bind 變數表 |
...
| 資料庫與合約 | backend 顯示 migration / contracts / workflow state；frontend 顯示 consumer contracts / Zod schemas / mock fixture 會放在哪裡 | 不可讓使用者自行推導 scope 條件 |

#### Q4 可修改項目

| Option | 使用者語意 | 會影響的 arguments 類型 |
|---|---|---|
| accept | 設定看起來合理，寫入 `${STACK_ROOT}/.aibdd/arguments.yml` | all |
| revise_layout | 修改 stack filesystem root（repo root vs subdirectory） | BACKEND_SUBDIR / FRONTEND_SUBDIR + 所有衍生路徑 |
...
| revise_database | 修改 migration / contracts / workflow state / frontend mock fixture 位置 | selected stack paths |
...
- <tlb id> 會有 contracts、data、shared/dsl.yml、packages（root）、test-strategy.yml（功能模組 `NN-<功能模組描述>/` 只在 `/aibdd-discovery` 綁定後建立）
...
#### Q4 白話範例（TypeScript / Next.js）

```markdown
請確認這些專案設定是否符合你的期待：

你選的是 TypeScript + Next.js，測試採 Playwright BDD + Storybook component contracts。
主要規格語言是 <project spec language>。
唯一 top-level boundary 會是 <tlb id>，同時作為 PROJECT_SLUG。

前端佈局：
- 前端 filesystem 根目錄會在 <frontend_root>
  （= ${PROJECT_ROOT}，或 ${PROJECT_ROOT}/${FRONTEND_SUBDIR} 當你選 subdir）
- 以下所有路徑都相對於這個根目錄

AIBDD 規格文件：
- 專案 config 會放在 <frontend_root>/.aibdd/arguments.yml
- 所有規格會放在 <frontend_root>/specs/
- 架構圖與 boundary 設定會放在 <frontend_root>/specs/architecture/
- 單次 plan package 的 spec、reports、clarifications 會放在 <frontend_root>/specs/<NNN-slug>/

功能與 boundary 文件：
- kickoff 會直接建立 <frontend_root>/specs/architecture/boundary.yml
- kickoff 會直接建立 <frontend_root>/specs/architecture/component-diagram.class.mmd
- <tlb id> 會有 contracts、data、shared/dsl.yml、packages（root）、test-strategy.yml（功能模組 `NN-<功能模組描述>/` 只在 `/aibdd-discovery` 綁定後建立）

Next.js 程式碼：
- App Router 會放在 <frontend_root>/src/app/
- components 會放在 <frontend_root>/src/components/
- api client / env / schemas 會放在 <frontend_root>/src/lib/
- Storybook stories 會放在 <frontend_root>/src/stories/

BDD 與 mock：
- playwright-bdd feature 檔會放在 <frontend_root>/features/
- step definitions 會放在 <frontend_root>/features/steps/
- fixtures 與 route mock 會是 <frontend_root>/features/steps/fixtures.ts
- Storybook 設定會放在 <frontend_root>/.storybook/

你想怎麼處理？
- Yes，接受並寫入 <frontend_root>/.aibdd/arguments.yml
- Revise layout：修改 frontend 根目錄（repo root 或子目錄）
- Revise specs：修改規格文件位置
- Revise source：修改程式碼位置
- Revise tests：修改測試位置
- Revise database：修改 contracts / schemas / mock fixture 位置
- Show technical details：顯示完整 arguments.yml
- Restart：重新確認前端設定
```
...
Stack 佈局：<derived layout label>（repo root 或 subdir <BACKEND_SUBDIR|FRONTEND_SUBDIR>）
參數：共 <N> 個

要修改哪一類設定？
- Stack 佈局（repo root vs subdirectory）
- 規格文件位置
- 程式碼位置
- BDD 測試位置
- migration / contracts / schemas / mock fixture 位置
- 重新選技術堆疊
...
已建立：
- <stack_root>/.aibdd/arguments.yml
- <stack_root>/specs/architecture/boundary.yml
- <stack_root>/specs/architecture/component-diagram.class.mmd
