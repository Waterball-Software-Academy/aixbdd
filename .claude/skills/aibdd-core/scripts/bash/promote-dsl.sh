#!/usr/bin/env bash
# Thin wrapper around .claude/skills/aibdd-core/lib/promote-dsl/ Python package.
# Usage:
#   promote-dsl.sh scan    --specs-root <specs/>
#   promote-dsl.sh propose --specs-root <specs/> --threshold <N>
#   promote-dsl.sh apply   --proposal <path> --dsl-core <path>
set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PKG_DIR="${SCRIPT_DIR}/../../lib/promote-dsl"

# Prefer uv-managed venv if available (zero-dep install: auto-resolves click/pyyaml).
if command -v uv >/dev/null 2>&1; then
  exec uv run --project "${PKG_DIR}" --quiet python -m promote_dsl.cli "$@"
fi
...
PYTHONPATH="${PKG_DIR}/src" exec "$PYEXE" -m promote_dsl.cli "$@"
