...
- **§5 與 `step-classification.yml` 對齊**（machine SSOT 指向）

**不放**：handler-selection 之 narrative 決策 SOP——已抽到 `decision-trees/*.yml`，結構化查表使用。

## §3 與 boundary preset routing（`/aibdd-plan` DSL）的關係

句型部位之機械對照表由 `aibdd-core/assets/boundaries/<preset>/step-classification.yml` 與其 `handlers/` 子樹擁有；per-handler 之 plan-time 履約則由 `plugin-contract.md` 承載。`/aibdd-plan` 透過 dsl_cli 將分類結果寫入扁平 entry 之 `handler` 欄。
