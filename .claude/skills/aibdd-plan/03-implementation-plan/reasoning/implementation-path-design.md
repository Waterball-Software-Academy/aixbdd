...
設計須考慮 SSOT：`${ACTIVITIES_DIR}/**`、`${FEATURE_SPECS_DIR}/**`；脈絡不足時補讀 `${PLAN_SPEC}`、`${PLAN_REPORTS_DIR}/discovery-sourcing.md`。另載入既有 operation／state 契約：`${CONTRACTS_DIR}/**`、`${DATA_DIR}/**`，以及 `${TRUTH_BOUNDARY_ROOT}/boundary-map.yml`（不存在則視為尚無 dispatch 覆寫）。

## Impact 覆蓋義務

`${PLAN_REPORTS_DIR}/discovery-sourcing.md` 之 Impact scope／Impact matrix 所列「本輪納入行為」，須與 `${CONTRACTS_DIR}/**` 中對客戶端暴露之 `operationId`（或同義路由入口）做交集盤點；交集內之每一入口視為須覆蓋之 boundary-entry operation。對每一須覆蓋之 operation，預設須各有 happy／alt／err 各至少一條獨立 path（檔粒度仍遵守 `sequence-path-granularity.md`）；若規格上某一類別無合理語意（例如僅唯一成功態），須於 `$IMPLEMENTATION_MODEL.blocked_reasons[]` 記載該 operation、缺漏類別與規格／契約依據，不得默認縮減全集。
