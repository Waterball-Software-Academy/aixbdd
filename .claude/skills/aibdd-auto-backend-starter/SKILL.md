...
description: 從 arguments.yml 與 templates 產生後端 walking skeleton。TRIGGER when 使用者說初始化後端、建後端骨架、backend starter、walking skeleton 後端，或被 kickoff 後續流程委派。SKIP when 需求是 feature-specific model/service/API/step definition 實作或既有專案重構。
metadata:
  user-invocable: true
...
Generate a backend walking skeleton from kickoff arguments and starter templates.
...
|---|---|---|---|
| R1 | `references/variants/python-e2e.md` | global | 定義 Python E2E starter（FastAPI + Behave + Alembic + Testcontainers）的 stack、placeholder、驗證與安全邊界 |
| R2 | `references/variants/java-e2e.md` | global | 定義 Java E2E starter（Spring Boot 4 + Cucumber + Flyway + Testcontainers + JdbcClient）的 stack、placeholder、驗證與安全邊界 |
...
8. BRANCH `$args_exists` ? GOTO #2.1 : GOTO #1.8.1
   8.1 `$missing_msg` = RENDER "`arguments.yml` 不存在；請先執行 /aibdd-kickoff"
   8.2 EMIT `$missing_msg` to user
   8.3 STOP

### Phase 2 — RESOLVE starter variant
> produces: `$$args_data`, `$$variant`, `$$variant_ref`, `$$variant_contract`
...
6. BRANCH `$legacy_kind`
   ...
   none       → GOTO #2.6.1
   6.1 `$missing_variant_msg` = RENDER "`arguments.yml` 缺少 `STARTER_VARIANT`；請填 `STARTER_VARIANT: python-e2e` 或 `STARTER_VARIANT: java-e2e`，或重新執行 /aibdd-kickoff"
   6.2 EMIT `$missing_variant_msg` to user
   6.3 STOP
7. `$unsupported_msg` = RENDER "目前 backend starter 支援 `python-e2e` / `java-e2e`；其他 variant 尚未實作。請改填其中之一，或重新執行 /aibdd-kickoff。"
8. EMIT `$unsupported_msg` to user
9. STOP
10. `$$variant` = COMPUTE `$variant_hint`
11. `$$variant_ref` = COMPUTE `references/variants/${$$variant}.md`
12. `$$variant_contract` = READ `$$variant_ref`
13. `$templates_dir` = COMPUTE `${$$skill_dir}/templates/${$$variant}`
14. `$templates_exist` = MATCH path_exists(`$templates_dir`)
15. BRANCH `$templates_exist` ? GOTO #3.1 : GOTO #2.15.1
   15.1 `$tmpl_msg` = RENDER "starter templates 不存在：`${$templates_dir}`"
   15.2 EMIT `$tmpl_msg` to user
   15.3 STOP
...
2. BRANCH `$payload_dir` ? GOTO #3.3 : GOTO #3.2.1
   2.1 [USER INTERACTION] `$$project_dir` = ASK "後端專案要建立在哪個目錄？預設為目前工作目錄。"
   2.2 GOTO #3.4
...
5. BRANCH `$payload_name` ? GOTO #3.6 : GOTO #3.5.1
   5.1 [USER INTERACTION] `$$project_name` = ASK "專案名稱？用於專案配置檔 (e.g., pyproject.toml / pom.xml) 與容器命名。"
   5.2 GOTO #3.7
...
13. BRANCH `$same_repo_args` ? GOTO #4.1 : GOTO #3.13.1
    13.1 `$repo_msg` = RENDER "backend starter 只讀同 repo 內既有 AIBDD project config；請在已 kickoff 的 backend root（= `${PROJECT_ROOT}/${BACKEND_SUBDIR}`，由 /aibdd-kickoff layout 決定）執行，且 arguments.yml 必須位於 `${project_dir}/${AIBDD_ARGUMENTS_PATH}`。若 layout 不對，請重跑 /aibdd-kickoff 並選擇正確的 backend 根目錄。"
    13.2 EMIT `$repo_msg` to user
    13.3 STOP
...
3. `$summary` = DRAFT plain-language summary from `$report`，並附帶：下游 `/aibdd-discovery` 必須以 `project_dir` 為工作目錄（`cwd`），使 `AIBDD_ARGUMENTS_PATH` / `BDD_CONSTITUTION_PATH` / `DEV_CONSTITUTION_PATH` 相對路徑與 `.aibdd/arguments.yml` 同框解析；starter 不產生、不改寫 `specs/`。
4. EMIT `$summary` to user

## §3 CROSS-REFERENCES

- `/aibdd-kickoff` — 上游產生 `arguments.yml` 與專案邊界設定
