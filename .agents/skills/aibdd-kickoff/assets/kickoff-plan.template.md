...
**組裝**：`## Questions` 段由 `assets/questions/q1..q6-*.template.md` 六支 question record 依序填入（Q5 data schema 僅 `type: web-service` 於第二輪填，其餘留空）；每支題目本文與 reply token 以該檔為 SSOT，本殼不重述題目細節。
