...
- `yes` — `install_spectrum = true`。auto-starter 會在 pom.xml 加入 specformula 依賴（`specformula-cucumber`、`specformula-testcontainer`、`specformula-dsl`，版本 0.0.6、test scope，從 Maven Central 取得）與 `specformula-dsl` preprocess plugin（`.dsl.feature` → `.isa.feature`）。
