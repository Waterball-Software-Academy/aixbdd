...
1. READ 讀取使用者需求、目標 plan package 的 `spec.md`、`plan.md`、`research.md`、`ui/**`、`truth-delta.md`，以及相關 `specs/truth/features/**`、`specs/truth/contracts/**`、`specs/truth/data/**`、`specs/truth/techstack.md`。
...
4. THINK 依 `spec.md` 的故事優先序、truth-delta rows、實際存在的 interface feature files、各介面 `dsl.md` 與 `plan.md` 的 shared dependency，收斂任務 phase 骨架；若關鍵 truth feature 或 DSL 缺失，停止受影響範圍，不自行發明第二套規格。
...
1. THINK 為每個 Feature File phase 填入 `Shared Must Read` 與 `Boundary`；truth 參照必須使用 `specs/truth/**` 路徑，plan 參照才使用當前 plan package 內相對路徑。
...
3. THINK 若 `spec.md` 的驗收情境無法被現有 truth feature/dsl 承接，停止受影響範圍並回交 `/dsl-refine`，不要新增沒有 DSL 的 acceptance feature file 來補洞。
