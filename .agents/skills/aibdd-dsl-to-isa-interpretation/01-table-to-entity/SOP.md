...
從 physical schema 規格（DBML 或 SQL DDL）抽出 table 名，產出 `${DATA_DIR}/entity_to_table_mapping.yml` — 後續 DSL-to-ISA 對齊與 SF dev framework `entity_setup` / `entity_validate` / `entity_non_existence_validate` 皆以本檔為唯一查表。
...
2. EXECUTE `build_mapping.py` — 直接執行 sibling script 產出 `${DATA_DIR}/entity_to_table_mapping.yml`；stdout/stderr 原樣 EMIT 給用戶，非 0 退出即 STOP。
   ...
   ```bash
   python3 .claude/skills/aibdd-dsl-to-isa-interpretation/01-table-to-entity/scripts/build_mapping.py "${DATA_DIR}"
   ```
