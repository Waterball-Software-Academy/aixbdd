...
14. 和使用者說道（詞可變、語意不變）：「我已經以 Discovery 真相把本輪須穩定保存的系統狀態推導成 data schema，並委派 specifier 落到 ${DATA_DIR}。你看一下我產的 DBML／資料規格確認設計沒問題；若要規劃本輪對外互動承諾（operation contract），可執行 /aibdd-api-plan。」
