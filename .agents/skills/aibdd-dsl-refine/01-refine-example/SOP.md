...
c+d. 逐個未完成 dsl_step（**一次一個，N 個 dsl_step → N 題，不批次**）：
   ...
   c. DERIVE＋CREATE dsl_step——在該 `{feature}.dsl.yml` 新建／補該 dsl_step（format `{name}` 佔位）：
      ...
      - 每條 isa_step 的 instruction 必對上主 SOP step 4 已載入的 isa 目錄某條 format；對不上且屬內建範圍外 → 依 `rules/custom-isa-placement.md` 在 FP 層 isa.yml 宣告契約（只寫契約，Step Definition 實作留 RED）。
