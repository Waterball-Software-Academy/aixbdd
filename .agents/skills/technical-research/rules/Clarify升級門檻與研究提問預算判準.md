...
# Rule 4 - 若 `spec.md` 已明確限制範圍，不得重問同一決策

- Level: `SHOULD`
- 當 `/specify` 已在 `spec.md` 中明確拍板 feature 邊界、第一版範圍、成功標準或不可違反限制時，`technical-research` 應直接沿用，不應把同一問題再次交給 `/clarify`。
- 只有在使用者最新指示與 `spec.md` 互相衝突，或 `spec.md` 對研究核心限制仍未定時，才應重啟訪談。
- 這樣可避免 skill chain 在 handoff 時重複詢問相同決策。

## Good Example

- 這個例子是好的，因為它直接沿用已在 spec 中拍板的第一版邊界。
