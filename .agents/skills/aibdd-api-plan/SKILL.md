...
4. RESOLVE `$BOUNDARY_PROFILE`——依 `aibdd-core::references/ssot/boundary-profile-resolution.md`「解析取值」就地取 `operation_contract_specifier.{skill,format}`，產出目錄對齊 `${CONTRACTS_DIR}`。
