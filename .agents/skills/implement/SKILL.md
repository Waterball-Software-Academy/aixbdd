...
- 若 task 帶 `[BDD-RED]`、`[BDD-ALIGN]`、`[BDD-GREEN]`、`[BDD-REFACTOR]`、`[BDD-REMOVE]`，必須呼叫 `/bdd` 的單一步驟，並明確帶上 truth delta action、truth feature file、對應 `dsl.md` 與既有測試對齊要求。
...
2. READ 依當前 task 載入必要 plan artifacts、`specs/truth/**` 與既有自動化測試落點；MODIFY / DELETE 類 task 必須載入或定位受影響 step definitions、fixtures、helpers、focused tests 與產品分支。
...
5. WRITE 依 task marker 完成本輪所需的程式、設定、文件或測試變更；不得越出本輪任務與 phase boundary，也不得讓過期測試繼續保護舊 truth。
